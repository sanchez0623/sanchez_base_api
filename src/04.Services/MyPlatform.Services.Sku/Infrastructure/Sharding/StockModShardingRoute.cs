...
/// 按SkuId取模分4表
/// </summary>
public class StockModShardingRoute : IShardingRouteRule<SkuStock>
{
    ...
    public StockModShardingRoute(IOptions<ShardingOptions> options)
    {
        _options = options.Value;
        _options.Tables.TryGetValue(nameof(SkuStock), out _tableConfig);
    }

    /// <inheritdoc />
    public string GetTableSuffix(object shardingValue)
    {
        var skuId = ConvertToLong(shardingValue);
        return (skuId % ShardCount).ToString();
    }
